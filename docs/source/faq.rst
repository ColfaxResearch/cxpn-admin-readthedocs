...
* ``wp_cc_external_api_logs`` -- External API request audit logs
...
This role is intended for portal operators who only need to manage the test drive portal and do not need access to general WordPress administration. A standard WordPress administrator sees the full dashboard with all menus. Both roles have the same underlying capabilities -- the difference is purely in the UI presentation.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
OTA is a setting that restricts users to making only one reservation during their entire project access period. When enabled, after the user's single reservation ends and feedback is submitted, their access concludes -- even if their access end date has not yet passed. When OTA is disabled, users can make multiple reservations as long as their project access is active.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Reservation times are displayed in the WordPress site's configured timezone (set in **WordPress Settings > General > Timezone**). All internal timestamps are stored as UTC epochs and converted to the site timezone for display. You can change the timezone at any time -- existing timestamps remain unchanged and are re-rendered in the new timezone.
